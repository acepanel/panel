...
const loadStreamServers = async () => {
  ...
  try {
    streamServers.value = await nginx.stream.listServers()
  } finally {
...
const loadStreamUpstreams = async () => {
  ...
  try {
    streamUpstreams.value = await nginx.stream.listUpstreams()
  } finally {
...
const handleSaveStreamServer = async () => {
  ...
  try {
    if (streamServerEditName.value) {
      await nginx.stream.updateServer(streamServerEditName.value, data)
    } else {
      await nginx.stream.createServer(data)
    }
    window.$message.success($gettext('Saved successfully'))
...
const handleDeleteStreamServer = async (name: string) => {
  try {
    await nginx.stream.deleteServer(name)
...
const handleSaveStreamUpstream = async () => {
  ...
  try {
    if (streamUpstreamEditName.value) {
      await nginx.stream.updateUpstream(streamUpstreamEditName.value, streamUpstreamModel.value)
    } else {
      await nginx.stream.createUpstream(streamUpstreamModel.value)
    }
    window.$message.success($gettext('Saved successfully'))
...
const handleDeleteStreamUpstream = async (name: string) => {
  try {
    await nginx.stream.deleteUpstream(name)
    window.$message.success($gettext('Deleted successfully'))
    loadStreamUpstreams()
  } catch (e: any) {
    window.$message.error(e.message || $gettext('Operation failed'))
